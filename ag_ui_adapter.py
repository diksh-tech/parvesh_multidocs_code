...
async def run_agent(request: Request):
    ...
    async def event_stream() -> AsyncGenerator[str, None]:
        ...
        plan_data = await mcp_client.plan_tools(user_query)
        ...
        for step_index, step in enumerate(plan):
            ...
            if isinstance(parsed,dict):
                candidate= parsed.get("data") if parsed.get("data") else parsed
                if isinstance(candidate,dict) and candidate.get("needs_route_selection"):
                    matches= candidate.get("matches") or candidate.get("documents") or []
                    yield sse_event({
                        "type": "MULTI_MATCHES",
                        "toolCallId":tool_call_id,
                        "matches": matches,
                        "message": "Multiple routes found - please let the user pick a doc_id to fetch details"
                    })
